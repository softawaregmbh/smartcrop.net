...
public class ViewModel : INotifyPropertyChanged
{
    ...
    private void Crop()
    {
        try
        {
            ...
            using (var bitmap = SKBitmap.Decode(this.SourceImagePath))
            {
                ...
                this.DebugImage = CreateImageSource(result.DebugInfo.Output);

                watch.Start();
                
                // crop the image
                SKRect cropRect = new(result.Area.Left, result.Area.Top, result.Area.Right, result.Area.Bottom);
                using (SKBitmap croppedBitmap = new((int)cropRect.Width, (int)cropRect.Height))
                using (SKCanvas canvas = new(croppedBitmap))
                {
                    SKRect source = new(cropRect.Left, cropRect.Top,
                                            cropRect.Right, cropRect.Bottom);
                    SKRect dest = new(0, 0, cropRect.Width, cropRect.Height);
                    canvas.DrawBitmap(bitmap, source, dest);
                    watch.Stop();

                    this.CroppedImage = CreateImageSource(croppedBitmap);
                };
            }
    ...
    private static BitmapImage CreateImageSource(SKBitmap bitmap)
    {
    ...
    private static BitmapImage CreateImageSource(byte[] bytes)
    {
